...
#
# RCS: @(#) $Id: Makefile,v 1.16 2000/08/31 14:45:08 awb Exp $
...
limit_LIB_FILE = limit30.dll
$(limit_LIB_FILE)_OBJECTS = $(limit_OBJECTS)

crypt_LIB_FILE = crypt30.dll
...
pkgdatadir = $(datadir)/tclhttpd3.0
pkglibdir = $(prefix)/lib/tclhttpd3.0
altpkg1libdir = $(libdir)/crypt3.0
altpkg2libdir = $(libdir)/limit3.0
pkgincludedir = $(includedir)/tclhttpd3.0
...
TCL_SRC_DIR = //Z/c2projects/connect2.0/tcl8.3/win
TCL_DBGX = 
...
DEFS =  -DVERSION=\"3.0\" -DBUILD_tclhttpd=1 -DUSE_TCL_STUBS=1  $(EXTRA_CFLAGS)
...
limit.$(OBJEXT): $(srcdir)/src/limit.c
	$(COMPILE) -c `cygpath -w $(srcdir)/src/limit.c` -o $@
...
install-lib-binaries: installdirs
	...
	$(INSTALL_SCRIPT) crypt_pkgIndex.tcl $(altpkg1libdir)/pkgIndex.tcl
	if test "$(BUILD_LIMIT)" = "1" ; then \
	    $(mkinstalldirs) $(altpkg2libdir) ; \
	    $(INSTALL_SCRIPT) limit_pkgIndex.tcl $(altpkg2libdir)/pkgIndex.tcl;\
	fi
